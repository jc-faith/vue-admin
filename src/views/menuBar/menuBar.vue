<template>
  <div class="menu-bar-container">
    <!-- logo -->
    <div class="logo" :class="$store.state.app.collapse?'menu-bar-collapse-width':'menu-bar-width'" :style="{'background':$store.state.app.themeColor}">
      <img :src="this.logo" /> <div>{{$store.state.app.collapse?'':sysName}}</div>
    </div>
    <!-- 导航菜单 -->
    <el-menu default-active="1-1" :class="$store.state.app.collapse?'menu-bar-collapse-width':'menu-bar-width'"
             :collapse="$store.state.app.collapse" @open="handleopen" @close="handleclose" @select="handleselect">
      <el-submenu index="1">
        <template slot="title">
